...
class ElbertConfigDownloader:
	...
	def SendData(self, Data):
		'The lowest level routine to send raw data to Elbert'
		'Returns total number of writes written'
		i = 0
		bytesWritten = 0;
		#Send data 30 bytes at a time. Elbert can recieve maximum 30 bytes per transaction
		while i < len(Data):
	...
	def SpiOpen(self):
		'Set up SPI peripheral inside PIC18 chip on Elbert'
	...
	def SpiPutChar(self, Char):
		...
		#START FIX Diego Herrera 17/09/2026
		if isinstance(Char, str):
				Char = ord(Char)
		#END FIX Diego Herrera 17/09/2026
	...
	def ConfigureElbert(self, FileName):
		'Configures Elbert'
...
#-------------------------- End class ElbertConfigDownloader -----------------#
